...
def generate_action(state_list, policy, action_bound):
    ...
    '''
    s_list.append(state_list[0])
    goal_list.append(state_list[1])
    speed_list.append(state_list[2])
...
def ppo_update(policy, optimizer, batch_size, memory, epoch, coeff_entropy=0.02, clip_value=0.2,
               num_step=2048, num_env=1, frames=1, obs_size=24, act_size=4):
